...
//int get_proc_info(int,struct proc_info*);
...
found:
  ...
  *(uint *)sp = (uint)trapret;
  ...
  p->shed_count = 0; // LAB4-TA2-gmeet -> initializes new counter fields to 0 for process 'p'
  return p;
}
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
      ...
      // LAB4-TA3-gmeet  -> increment its shed_count field right before context switch
      p->shed_count++;
...
int get_proc_info(int pid, struct proc_info *info)
{
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    if (p->pid == pid) {
      ...
      info->pid = p->pid;
      info->sz = p->sz; // This correctly assigns the 32-bit uint to the 64-bit uint64.
